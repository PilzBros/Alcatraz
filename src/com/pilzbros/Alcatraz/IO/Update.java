	...
	public void query() {
        ...
        try {
            ...
            if (array.size() > 0) 
            {
                ...
                if (Double.parseDouble(versionName) > Double.parseDouble(Alcatraz.pluginVersion))
                {
                	Alcatraz.log.log(Level.WARNING, Alcatraz.consolePrefix + " Update available!");
	                Alcatraz.updateNeeded = true;
                }
                else
                {
	                Alcatraz.updateNeeded = false;
                }
                
            } 
        } catch (IOException e) {
            // There was an error reading the query
        	Alcatraz.log.log(Level.WARNING, Alcatraz.consolePrefix + " There was an issue reading the update information...");
            e.printStackTrace();
            return;
        }
    }
